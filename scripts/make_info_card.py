...
# ("host",)                    -> "pranjal@github" + rule
# ("kv", key, value)           -> purple key + light value
# ("sec", title)               -> purple section rule
# ("bul", text)                -> green dot + light text
# ("gap",)                     -> vertical space
ROWS = [
    ("host",),
    ("kv", "Focus", "Machine Learning + Applied AI"),
    ("kv", "Build", "Production data and vision systems"),
    ("kv", "Research", "HALE prediction and health analytics"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Core", "Python, Jupyter, R, SQL"),
    ("kv", "ML", "scikit-learn, computer vision"),
    ("kv", "Systems", "FastAPI, Docker, dashboards"),
    ("kv", "Methods", "statistics, forecasting, risk scoring"),
    ("gap",),
    ("sec", "Featured work"),
    ("bul", "Learn2Drive - autonomy ML and safety"),
    ("bul", "DefectScope - manufacturing vision"),
    ("bul", "NetPulse - behavioral risk scoring"),
    ("bul", "HALE - machine-learning research"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">pranjal@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">pranjal</tspan><tspan fill="{MUTED}">@</tspan>'
